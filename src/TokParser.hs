...
testWord tok = case tok of
  Word w -> Just w
  _ -> Nothing

testOp :: Token -> Maybe Token
testOp tok = case tok of
  Word w -> Nothing
  NEWLINE -> Nothing
  EOF -> Nothing
  t -> Just t
...
getWordC extraCheck = tokenPrim show nextPosW (extraCheck <=< checkIfRes <=< testWord)
  where
    checkIfRes w = guard (w `notElem` (fst <$> reservedWords)) $> w
...
parseIORed =
  ((\op -> Redirect op (getDefOp op)) <$> getRedirOp <*> getWord) <|> do
    n <- getIONr
    redirOp <- getRedirOp
    Redirect redirOp (Fd . fromIntegral $ n) <$> getWord
  where
    getRedirOp = tokenPrim show nextPosTok ((>>= isRedirOp) <$> testOp)
    defaultFd =
      [ (LESS, stdInput),
        (GREAT, stdOutput),
        (CLOBBER, stdOutput),
...
parseSmpCmd =
  addRedirect <$> try parseIORed <*> (parseSmpCmd <|> base)
    <|> addAssign <$> getAssignWord <*> (parseSmpCmd <|> base)
    <|> addWord <$> getWord <*> parseSmpCmdSuf
  where
    delimit = tokenPrim show nextPosTok (guard . (/= LBracket))
    ...
    parseSmpCmdSuf =
      addRedirect <$> try parseIORed <*> parseSmpCmdSuf
        <|> addWord <$> getWord <*> parseSmpCmdSuf
        <|> base

parseList :: (a -> TokParser (a, Token)) -> Bool -> [Token] -> TokParser a -> TokParser [(a, Token)]
parseList endCondition nLs sepToks parseElem =
  parseElem
...
parseFuncDef = FuncDef <$> getWordC funcDef <* (op LBracket >> op RBracket) <*> parseBody
  where
    funcDef n = case parse parseXBDName "parsename" n of
...
parseSepList nLs seps = parseList endCondition nLs seps parseAndOrList
  where
    endCondition e = ((e,) <$> oneOfOp seps <* when nLs newLnList) <|> return (e, EOF)
...
parseIfClause initKeyW = join $ handler <$> (resWord initKeyW *> parseCmpList) <*> (resWord "then" *> parseCmpList)
  where
    appendElif condition body cl_elif = return $ cl_elif {clauses = (condition, body) : clauses cl_elif}
    handler cond body =
      (parseIfClause "elif" >>= appendElif cond body)
