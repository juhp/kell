...
quoteEsc recCondition endCondition =
  endCondition <|> (eof >> unexpected "eof")
...
getDollarExp f s = foldl1 (<|>) (stackHandler <$> stackAction s) -- Pattern matching will fail if string is empty
  where
    closingAction s c =
      if stackIsEmpty s || (stackPeek s /= Just c)
        then Nothing
...
data ShellEnv = ShellEnv
  { var :: Map.Map String (String, Bool),
...
getErrExitCode e = case e of
  SyntaxErr _ -> ExitFailure 117
...
getVar name = do
  env <- lift get
  case Rd.readMaybe name :: Maybe Int of
    Just n -> return $ getPos env n --TODO exclude negative n
    _ -> lift get <&> getVal . Map.lookup name . var
  where
    getVal entry = case entry of
      (Just (val, _)) -> Just val
      _ -> Nothing
    getPos env n = case stackPeek (posArgs env) of
      Just args ->
        if length args < n
          then Nothing
          else Just $ args !! (n -1)
...
putFunc (FuncDef name cmd) = lift get >>= lift . put . changeNamespace (Map.insert name cmd . func)
  where
    changeNamespace modifier curEnv = curEnv {func = modifier curEnv}
...
putVar name newval = do
  ...
  case oldentry of
    (Just (_, True)) -> liftIO $ setEnv name newval
    _ -> return ()
  lift get >>= lift . put . changeNamespace (Map.insert name (newEntry newval oldentry) . var)
  where
    changeNamespace modifier curEnv = curEnv {var = modifier curEnv}
    newEntry newval oldentry = case oldentry of
      (Just (_, True)) -> (newval, True)
      _ -> (newval, False)
